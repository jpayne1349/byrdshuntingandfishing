<script lang="ts">
  ...
  let photoPagesList = ["hunting", "fishing", "hidden"];
  ...
  function firstPhotoUrlOrStock(category: string) {
    ...
    for (let i = 0; i < $photoStore.all.length; i++) {
      // if category matches, check to make sure it's not a hidden photo and then asign
      if ($photoStore.all[i].category == category) {
        if ($photoStore.all[i].hidden) continue;
        firstPhoto = $photoStore.all[i];
        break;
      }
      // above will never trigger first on hidden photo, check here and assign to first found
      if (category == "hidden") {
        if ($photoStore.all[i].hidden) {
          firstPhoto = $photoStore.all[i];
          break;
        }
      }
...
<div class="container grid min-h-[85vh] grid-cols-2 lg:grid-cols-6 gap-6 auto-rows-min">
  {#if photoLinksGenerated}
    {#each photoPagesList as pageName, index}
      <a href="/cms/photos/{pageName}" class="text-center h-fit">
        ...
        <h3>{toTitleString(pageName)} Photos</h3>
      </a>
    {/each}
  {:else}
    {#each photoPagesList as pageName, index}
      <a href="/cms/photos/{pageName}" class="text-center">
